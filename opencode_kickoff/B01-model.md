...
| 对象 | 代码坐标 | 在系统里的角色 |
| --- | --- | --- |
| `Agent.Info` | `packages/opencode/src/agent/agent.ts:25-50` | 定义 agent 的静态能力边界：模式、权限、默认模型、prompt、steps。 |
| 内建 agents | `agent.ts:77-203` | `build`、`plan`、`general`、`explore`、`compaction`、`title`、`summary`。 |
...
`agent.ts:77-203` 里当前内建 agents 的角色很清楚：
...
`206-248` 会把 `config.agent` 覆写到默认 agent 上。可以改的是字段值，不能改的是模型：
