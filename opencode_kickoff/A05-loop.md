...
`packages/opencode/src/session/prompt.ts:278-736` 可以粗分成 6 段：

1. 占用/恢复 session 运行态，见 `281-289`。
2. 每轮回放历史、求 `lastUser` / `lastAssistant` / `lastFinished` / `tasks`，见 `296-319`。
3. subtask 分支，见 `354-529`。
4. compaction 分支，见 `532-544`。
5. overflow 检测与普通推理分支，见 `546-724`。
6. 收尾：prune、返回最新 assistant、唤醒排队调用者，见 `726-735`。
...
如果 `task?.type === "subtask"`，`356-529` 会进入专门分支。
...
如果 `task?.type === "compaction"`，`532-544` 会直接调用：
...
`546-559` 会在最近一条已完成 assistant 上做 overflow 检测：
...
当没有 pending subtask/compaction，且没有触发 overflow 时，才进入 `561-724` 的 normal processing。
...
`565-569` 的 `insertReminders()` 会根据 plan/build 模式补 synthetic reminder；之后 `571-600` 会先落 assistant skeleton message。

这意味着普通推理轮次的 assistant 头信息始终先 durable，再开始流式生成。

### 8.3 tool set 是每轮重新解析的

`607-615` 调 `resolveTools(...)`，它会根据：
...
若 `lastUser.format.type === "json_schema"`，`617-625` 会临时注入 `StructuredOutput` 工具，并把 `toolChoice` 设成 `"required"`。

所以 OpenCode 的结构化输出不是特判一个 provider API，而是把“最终答案必须走工具”编译成统一工具流。

### 8.5 queued user message 会被临时包上 system-reminder

`634-650` 会把上一条已完成 assistant 之后、尚未处理的 user text 包成：
