...
flowchart TD
    ...
    class BusMsg,BusPart,BusDelta consumer;
...
flowchart LR
    ...
    class DeltaIn producer;
    class Realtime consumer;
    class Broadcast,Snapshot bridge;
...
flowchart LR
    ...
    class Start,Delta,End producer;
    class Placeholder,Realtime,Final bridge;
```
...
flowchart TD
    ...
    class Finish,Usage,Msg producer;
    class Overflow consumer;
    class StepPart,Patch,PatchPart,Summary bridge;
```
...
flowchart LR
    ...
    class Def,Payload producer;
    class Local,Wildcard,Global consumer;
```
...
这里的生产者还是同一个 `Bus.publish()`，只是它在通知本地订阅者之后，还会继续 `GlobalBus.emit("event", ...)`。
...
flowchart LR
    ...
    class Global,Route,SDK,Emit bridge;
```
